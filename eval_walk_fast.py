...
def main():
    ...
    # reward_cfg["reward_scales"] = {} # TODO not sure if this is needed
    env = Go2(
        num_envs=1,
        env_cfg=env_cfg,
        obs_cfg=obs_cfg,
    ...
    from train_walk_fast import get_train_cfg
    ...
    with torch.no_grad():
        ...
        while not stop:
            ...
            if args.record:
                if n_frames == 100:
                    env.stop_recording("run_fast.mp4")
                    exit()
...
"""
python eval_walk_fast.py -e walk_fast_v1 -c --ckpt 400
"""
