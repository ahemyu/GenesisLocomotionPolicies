...
def get_cfgs():
    ...
    reward_cfg = {
        ...
        'reward_scales': {
            'tracking_lin_vel': 2.0,    # Increased from 1.0 to emphasize speed
            'tracking_ang_vel': 0.5,
            'lin_vel_z': -2.0,
            'ang_vel_xy': -0.05,
            'orientation': -10.0,
            'base_height': -50.0,
            'torques': -0.0001,         # Reduced penalty from -0.0002
            'collision': -1.0,
            'dof_vel': 0.0,
            'dof_acc': -1e-7,           # Reduced penalty from -2.5e-7
            'feet_air_time': 2.0,       # Increased from 1.0 for longer strides
            'action_rate': -0.005,      # Reduced penalty from -0.01
        },
    }
    command_cfg = {
        'num_commands': 4,
        'lin_vel_x_range': [1.0, 3.0],  # Encouzrage faster forward speed
        'lin_vel_y_range': [-0.2, 0.2],  # lower lateral movement
        'ang_vel_range': [-0.1, 0.1],    # lower turning
    }
    return env_cfg, obs_cfg, reward_cfg, command_cfg

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('-e', '--exp_name', type=str, default='walk_fast')
...
'''
python train_walk_fast.py -e walk_fast_v1 
'''